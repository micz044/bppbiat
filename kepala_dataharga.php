...
<div class="main scroll-container">
    ...
    <section id="persediaan" class="main-section active">
            ...
            <table id="transaksiTable">
                ...
                $sql_dataharga = "SELECT id, jenis_benih, ukuran, harga FROM data_harga";
                $result_dataharga = $conn->query($sql_dataharga);

                if ($result_dataharga->num_rows > 0) {
                    while($sql_dataharga = $result_dataharga->fetch_assoc()) {
                        echo "<tr data-id='" . $sql_dataharga['id'] . "'>
                        <td>" . $sql_dataharga['id'] . "</td>
                        <td>" . $sql_dataharga['jenis_benih'] . "</td>
                        <td>" . $sql_dataharga['ukuran'] . " Cm</td>
                        <td>Rp " . number_format($sql_dataharga['harga'], 0, ',', '.') . "</td>
                        </tr>";
                    }
                } else {
                    echo "<tr><td colspan='3'>Tidak ada data harga.</td></tr>";
                }
                ?>
                </tbody>
            </table>
            ...
            <form action="KFtambahdataharga_kepala.php" method="post">
                ...
                <button type="submit" name="submit_add_data_harga">Tambah</button>
            </form>

            <?php
            if (isset($_GET['success'])) {
                $success = $_GET['success'];
                if ($success == 'berhasil_tambah') {
                    echo "<div class='notification success' id='notification'>Berhasil menambah data harga.</div>";
                }
            }
            ?>

            <!-- Form untuk menghapus data tertentu di data_persediaan -->
            <form method="POST" action="KFhapusdataharga_kepala.php">
...
<?php
    if (isset($_GET['success'])) {
        $success = $_GET['success'];
        if ($success == 'edit_tabeldh_berhasil') {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        tableRows.forEach(function(row) {
            row.addEventListener('click', function() {
                ...
                if (userConfirmed) {
                    window.location.href = 'kepala_edit_dataharga.php?id=' + id;
                }
            });
        });
    });
